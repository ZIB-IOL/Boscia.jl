...
function merge_csvs(;example="sparse_reg", seeds=1:10, dimensions=15:30, Ns=[], k=[], var_A=[])
    ...
    if example == "sparse_reg" 
        df = DataFrame(CSV.File(joinpath(@__DIR__, "csv/Ipopt/ipopt_" * example * "_1_17.csv")))
    elseif occursin("miplib", example) 
        df = DataFrame(CSV.File(joinpath(@__DIR__, "csv/Ipopt/ipopt_" * example * "_" * string(dimensions[1]) * "_" * string(seeds[1]) * ".csv")))
    ...
    if example == "sparse_reg" 
        for seed in seeds 
            for dimension in dimensions 
                try 
                    df_temp = DataFrame(CSV.File(joinpath(@__DIR__, "csv/Ipopt/ipopt_" * example * "_" * string(seed) * "_" * string(dimension) * ".csv")))
                    append!(df, df_temp)
                catch e 
                    println(e)
                end 
    ...
    elseif occursin("miplib", example) 
        for seed in seeds 
            for dimension in dimensions 
                try 
                    df_temp = DataFrame(CSV.File(joinpath(@__DIR__, "csv/Ipopt/ipopt_" * example * "_" * string(dimension) * "_" * string(seed) * ".csv")))
                    append!(df, df_temp)
                catch e 
                    println(e)
                end 
            ...
            for seed in seeds
                for ns in Ns
                    for var in var_A
                        k = Float64(dimension)
                        try 
                            df_temp = DataFrame(CSV.File(joinpath(@__DIR__, "csv/Ipopt/ipopt_" * example * "_" * string(seed) * "_" * string(dimension) * "_" * string(var) * "_" * string(dimension*5) * "_" * string(ns) * ".csv")))
                            append!(df, df_temp)
                        catch e
                        print(e)
                        end
    ...
    elseif example == "poisson_reg"
        for dimension in dimensions
            ...
            for seed in seeds
                for ns in Ns
                    try 
                        df_temp = DataFrame(CSV.File(joinpath(@__DIR__, "csv/Ipopt/ipopt_" * example * "_" * string(seed) * "_" * string(dimension) * "_" * string(float(p/2)) * "_" * string(ns) * ".csv")))
                        append!(df, df_temp)
                    catch e 
                        println(e)
                    end
