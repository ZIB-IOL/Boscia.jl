...
function dual_gap_plot(file_name, mode)
    ...
    lns1 = ax.plot(1:len, df[!,"ub"], label="Incumbent", color=cb_green_sea, marker=markers[2], markevery=0.05)
    lns2 = ax.plot(1:len, df[!,"lb"], label="Lower bound", color=cb_clay, marker=markers[3], markevery=0.05)
    ...
    ax2 = ax.twinx()
    lns3 = ax2.plot(1:len, df[!, "list_lmo_calls"], label="Total BLMO calls", color=cb_lilac, marker=markers[1], markevery=0.05)
