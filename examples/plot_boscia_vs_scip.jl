...
function plot_boscia_vs_scip(mode; afw=false, ss=false, as=false, as_ss=false)
    ...
    if ss 
        if mode == "mixed_50"
            df_afw = DataFrame(CSV.File(joinpath(@__DIR__, "csv/no_warm_start_ss_mixed_50.csv")))
            filter!(row -> !(row.time_afw >= time_limit),  df_afw)
            time_afw = sort(df_afw[!,"time_afw"])
            push!(time_afw, 1.1 * time_limit)
            ax.plot(time_afw, [1:nrow(df_afw); nrow(df_afw)], label="no shadow set", color=colors[5], marker=markers[4])
        elseif mode == "integer_50"
            df_afw = DataFrame(CSV.File(joinpath(@__DIR__, "csv/no_warm_start_ss_integer_50.csv")))
            filter!(row -> !(row.time_afw >= time_limit),  df_afw)
            time_afw = sort(df_afw[!,"time_afw"])
            push!(time_afw, 1.1 * time_limit)
            ax.plot(time_afw, [1:nrow(df_afw); nrow(df_afw)], label="no shadow set", color=colors[5], marker=markers[4])
