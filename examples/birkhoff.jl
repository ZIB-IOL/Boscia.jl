            ...
            for i in 1:iter
                for (idx,eps) in enumerate(fw_epsilon_values)
                    for (idx2, min_num_lower_val) in enumerate(min_num_lower_values)
                        ...
                        file_name = "csv/early_stopping_" * example * "_" * string(n) * "_" * string(k) * "_" * string(seed) * "_" * string(min_number_lower) * "_" * string(dual_gap_decay_factor) * "_" * string(fw_epsilon) * "_" * string(i) *".csv"
                        @show file_name
                        CSV.write(file_name, df, append=false)
                    end
                end
