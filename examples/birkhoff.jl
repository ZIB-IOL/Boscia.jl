...
function birkhoff(dual_gap_decay_factor, min_number_lower, fw_epsilon, seed=1, n=2, k=2)
    @show seed
    Random.seed!(seed)
...
function birkhoff_data()
    example = "birkhoff"

    values = 0.7 #[0.9, 1.0]
    fw_epsilon_values = 1e-3#[1e-3, 5e-3, 1e-4, 1e-7]
    min_num_lower_values = Inf#[20, 40, 60, 80, 100, 200, Inf]
    seeds = 1#[1]#,2,3]
            ...
            for i in 1:iter
                for (idx,eps) in enumerate(fw_epsilon_values)
                    for (idx2, min_num_lower_val) in enumerate(min_num_lower_values)
                        ...
                        fw_epsilon = eps
                        seed = seed_val
                        data, result = birkhoff(dual_gap_decay_factor, min_number_lower, fw_epsilon, seed)
                        ...
                        file_name = "final_csvs/early_stopping_" * example * "_" * string(n) * "_" * string(k) * "_" * string(seed) * "_" * string(min_number_lower) * "_" * string(dual_gap_decay_factor) * "_" * string(fw_epsilon) * "_" * string(i) *".csv"
                        @show file_name
                        CSV.write(file_name, df, append=false)
                    end
                end
