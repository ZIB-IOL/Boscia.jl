...
function sparse_reg_boscia(seed=1, n=5; full_callback=false, bo_mode="default", depth=1, write=true, dual_gap_decay_factor=0.0, fw_epsilon=0.0)
   ...
   # print(lmo.o)
    ...
    elseif bo_mode == "dual_gap_decay_factor"
        x, _, result = Boscia.solve(f, grad!, lmo, verbose=true, use_postsolve=false, dual_gap_decay_factor=dual_gap_decay_factor, fw_epsilon=fw_epsilon)
    else
    ...
    if full_callback
        ...
        elseif bo_mode == "dual_gap_decay_factor"
            file_name = joinpath(@__DIR__,"csv/Boscia/boscia_" * bo_mode * "_sparse_reg_" * string(seed) * "_" * string(n) * "_" * string(dual_gap_decay_factor) * "_" * string(epsilon) * ".csv")
        else 
