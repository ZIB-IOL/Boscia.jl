...
function portfolio(seed=1, dimension=5; mode, bo_mode)
    ...
    df = DataFrame(seed=seed, dimension=n, time=total_time_in_sec, solution=result[:primal_objective], rel_dual_gap=result[:rel_dual_gap], termination=status, ncalls=result[:lmo_calls])
    if bo_mode ==  "afw"
