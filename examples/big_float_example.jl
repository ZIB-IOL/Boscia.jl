...
@testset "Using BigFloat" begin
    ...
    x, _, result =
        Boscia.solve(f, grad!, sblmo, lbs[int_vars], ubs[int_vars], int_vars, n, verbose=true, time_limit=300, custom_heuristics = custom_heuristics)

    if result[:total_time_in_sec] < 910
        @test x == round.(diffi)
    end
