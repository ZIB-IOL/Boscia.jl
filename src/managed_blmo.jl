...
function is_decomposition_invariant_oracle(sblmo::CubeSimpleBLMO, blmo::ManagedBoundedLMO)
    ...
    if !(distinct_lbs = indicator1) && !(distinct_lbs = indicator2)
        return false
    end
    if !(distinct_ubs = indicator1) && !(distinct_ubs = indicator2)
        return false
    end
...
"""
is_decomposition_invariant_oracle(sblmo::ProbablitySimplexSimpleBLMO, blmo::ManagedBoundedLMO) = (sblmo.N === 1.0)
...
is_decomposition_invariant_oracle(::UnitSimplexSimpleBLMO, blmo::ManagedBoundedLMO) = (sblmo.N === 1.0)
