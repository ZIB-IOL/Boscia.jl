...
@testset "Poisson sparse regression" begin
    ...
    m = BranchAndBound.SimpleOptimizationProblem(f, grad!, 2p+1, collect(p+1:2p), time_lmo, global_bounds) 
    ...
    Node = typeof(nodeEx),
    root = (problem=m, current_node_id = Ref{Int}(0), options= Dict{Symbol, Any}(:FW_tol => 1e-4, :verbose => false)),
    )
...
@testset "Sparse Group Poisson" begin
    ...
    time_lmo = BranchAndBound.TimeTrackingLMO(lmo)
    active_set = FrankWolfe.ActiveSet([(1.0, v)]) 
    m = BranchAndBound.SimpleOptimizationProblem(f, grad!, 2pg+1, collect(pg+1:2pg), time_lmo, global_bounds) 
...
@testset "Strong branching sparse group poisson" begin
    ...
    tree = Bonobo.initialize(;
        traverse_strategy = Bonobo.BFS(),
        Node = typeof(nodeEx),
        root = (problem=m, current_node_id = current_node_id = Ref{Int}(0), options= Dict{Symbol, Any}(:FW_tol => 1e-4, :verbose => false)),
        branch_strategy = branching_strategy, #() ->
    )
    Bonobo.set_root!(tree, 
    (active_set = active_set, 
