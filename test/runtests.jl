...
@testset "BnBTree data structure and node" begin
    ...
    tree = Bonobo.initialize(; 
        ...
        root = (problem=m, current_node_id = Ref{Int}(0), options= Dict{Symbol, Any}(:verbose => false, :dual_gap_decay_factor => 0.7,  :dual_gap => 1e-6, :max_fw_iter => 10000)),
    )
...
@testset "IP vs LP solver" begin
    ...
    tree = Bonobo.initialize(; 
        traverse_strategy = Bonobo.BFS(),
        Node = typeof(nodeEx),
        root = (problem=m, current_node_id = current_node_id = Ref{Int}(0), options= Dict{Symbol, Any}(:dual_gap_decay_factor => 0.7, :verbose => false, :dual_gap => 1e-6, :max_fw_iter => 10000)),
    )
    Bonobo.set_root!(tree, 
        (active_set = active_set, 
        discarded_vertices = vertex_storage,
